...
for SSH_ADDRESS in "${SSH_ADDRESSES[@]}"; do
    rsync -avz "$TRAJ" "$SSH_ADDRESS":
    rsync -avz "sensor/sensor.img" "$SSH_ADDRESS":
    ...
    ssh "$SSH_ADDRESS" sudo rsync sensor.img /celestial/sensor.img
...
ssh "$COORDINATOR_ADDRESS" python3 celestial.py $TRAJ $HOSTS > coordinator.log 2>&1  &
